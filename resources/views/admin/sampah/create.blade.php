                ...
                <form action="{{ route('admin.sampah.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kategori') is-invalid @enderror" 
                            ...
                            <option value="non-organik" {{ old('kategori') == 'non-organik' ? 'selected' : '' }}>Non-Organik</option>
                        </select>
                        @error('kategori')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('harga_per_kg') is-invalid @enderror" 
                               id="harga_per_kg" name="harga_per_kg" value="{{ old('harga_per_kg') }}" required min="0">
                        @error('harga_per_kg')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('poin_per_kg') is-invalid @enderror" 
                               id="poin_per_kg" name="poin_per_kg" value="{{ old('poin_per_kg') }}" required min="0">
                        @error('poin_per_kg')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
